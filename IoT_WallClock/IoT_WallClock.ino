...
// =============================================================================
// 5×8 비트맵 폰트 (ASCII 0x20 ~ 0x7E, 95글자) — HD44780 스타일 8행 풀-스케일
//   bit4 = 왼쪽, bit0 = 오른쪽 (5비트 유효), 각 글자 8행
//   시계 숫자(drawDigit) / 스크롤 텍스트(drawStringPx) 공통 사용
//   대문자·숫자: 행 0~7 풀, HD44780 디자인 디테일(둥근 0 가운데 점,
//   사다리꼴 D, 꼬리 있는 9 등) 유지하면서 1행씩 늘려서 매트릭스 채움
//   소문자: x-height 6행, ascender 8행 풀, descender(g·j·p·q·y) 행 7 사용
// =============================================================================
#define FONT_5X8_FIRST   0x20
#define FONT_5X8_LAST    0x7E
#define FONT_5X8_W       5
#define FONT_5X8_H       8
#define FONT_X_ADVANCE   6   // 글자 폭 5 + 글자 간격 1

const uint8_t PROGMEM FONT_5X8[95][8] = {
  { 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 }, // 0x20 ' '
  { 0x04,0x04,0x04,0x04,0x04,0x04,0x00,0x04 }, // 0x21 '!'
  { 0x0A,0x0A,0x0A,0x00,0x00,0x00,0x00,0x00 }, // 0x22 '"'
  { 0x0A,0x0A,0x1F,0x0A,0x0A,0x1F,0x0A,0x0A }, // 0x23 '#'
  { 0x04,0x0F,0x14,0x0E,0x05,0x05,0x1E,0x04 }, // 0x24 '$'
  { 0x18,0x19,0x02,0x04,0x04,0x08,0x13,0x03 }, // 0x25 '%'
  { 0x08,0x14,0x14,0x08,0x08,0x15,0x12,0x0D }, // 0x26 '&'
  { 0x0C,0x04,0x08,0x00,0x00,0x00,0x00,0x00 }, // 0x27 '\''
  { 0x02,0x04,0x08,0x10,0x10,0x08,0x04,0x02 }, // 0x28 '('
  { 0x08,0x04,0x02,0x01,0x01,0x02,0x04,0x08 }, // 0x29 ')'
  { 0x00,0x00,0x04,0x15,0x0E,0x15,0x04,0x00 }, // 0x2A '*'
  { 0x00,0x04,0x04,0x04,0x1F,0x04,0x04,0x04 }, // 0x2B '+'
  ...
  { 0x01,0x01,0x02,0x02,0x04,0x08,0x08,0x10 }, // 0x2F '/'
  { 0x0E,0x11,0x11,0x13,0x15,0x19,0x11,0x0E }, // 0x30 '0'
  { 0x04,0x0C,0x04,0x04,0x04,0x04,0x04,0x0E }, // 0x31 '1'
  { 0x0E,0x11,0x01,0x01,0x02,0x04,0x08,0x1F }, // 0x32 '2'
  { 0x1F,0x01,0x02,0x04,0x02,0x01,0x11,0x0E }, // 0x33 '3'
  { 0x02,0x06,0x0A,0x12,0x12,0x1F,0x02,0x02 }, // 0x34 '4'
  { 0x1F,0x10,0x10,0x1E,0x01,0x01,0x11,0x0E }, // 0x35 '5'
  { 0x06,0x08,0x10,0x10,0x1E,0x11,0x11,0x0E }, // 0x36 '6'
  { 0x1F,0x01,0x02,0x02,0x04,0x04,0x08,0x08 }, // 0x37 '7'
  { 0x0E,0x11,0x11,0x0E,0x11,0x11,0x11,0x0E }, // 0x38 '8'
  { 0x0E,0x11,0x11,0x11,0x0F,0x01,0x02,0x0C }, // 0x39 '9'
  { 0x00,0x00,0x0C,0x0C,0x00,0x0C,0x0C,0x00 }, // 0x3A ':'
  { 0x00,0x00,0x0C,0x0C,0x00,0x0C,0x04,0x08 }, // 0x3B ';'
  { 0x01,0x02,0x04,0x08,0x10,0x08,0x04,0x02 }, // 0x3C '<'
  { 0x00,0x00,0x1F,0x00,0x00,0x1F,0x00,0x00 }, // 0x3D '='
  { 0x10,0x08,0x04,0x02,0x01,0x02,0x04,0x08 }, // 0x3E '>'
  { 0x0E,0x11,0x01,0x02,0x04,0x04,0x00,0x04 }, // 0x3F '?'
  { 0x0E,0x11,0x01,0x0D,0x15,0x15,0x15,0x0E }, // 0x40 '@'
  { 0x0E,0x11,0x11,0x11,0x1F,0x11,0x11,0x11 }, // 0x41 'A'
  { 0x1E,0x11,0x11,0x11,0x1E,0x11,0x11,0x1E }, // 0x42 'B'
  { 0x0E,0x11,0x10,0x10,0x10,0x10,0x11,0x0E }, // 0x43 'C'
  { 0x1C,0x12,0x11,0x11,0x11,0x11,0x12,0x1C }, // 0x44 'D'
  { 0x1F,0x10,0x10,0x10,0x1E,0x10,0x10,0x1F }, // 0x45 'E'
  { 0x1F,0x10,0x10,0x10,0x1E,0x10,0x10,0x10 }, // 0x46 'F'
  { 0x0E,0x11,0x10,0x10,0x10,0x17,0x11,0x0F }, // 0x47 'G'
  { 0x11,0x11,0x11,0x11,0x1F,0x11,0x11,0x11 }, // 0x48 'H'
  { 0x0E,0x04,0x04,0x04,0x04,0x04,0x04,0x0E }, // 0x49 'I'
  { 0x07,0x02,0x02,0x02,0x02,0x02,0x12,0x0C }, // 0x4A 'J'
  { 0x11,0x12,0x14,0x18,0x18,0x14,0x12,0x11 }, // 0x4B 'K'
  { 0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x1F }, // 0x4C 'L'
  { 0x11,0x1B,0x15,0x15,0x15,0x11,0x11,0x11 }, // 0x4D 'M'
  { 0x11,0x11,0x19,0x15,0x15,0x13,0x11,0x11 }, // 0x4E 'N'
  { 0x0E,0x11,0x11,0x11,0x11,0x11,0x11,0x0E }, // 0x4F 'O'
  { 0x1E,0x11,0x11,0x11,0x1E,0x10,0x10,0x10 }, // 0x50 'P'
  { 0x0E,0x11,0x11,0x11,0x11,0x15,0x12,0x0D }, // 0x51 'Q'
  { 0x1E,0x11,0x11,0x11,0x1E,0x14,0x12,0x11 }, // 0x52 'R'
  { 0x0E,0x11,0x10,0x10,0x0E,0x01,0x11,0x0E }, // 0x53 'S'
  { 0x1F,0x04,0x04,0x04,0x04,0x04,0x04,0x04 }, // 0x54 'T'
  { 0x11,0x11,0x11,0x11,0x11,0x11,0x11,0x0E }, // 0x55 'U'
  { 0x11,0x11,0x11,0x11,0x11,0x11,0x0A,0x04 }, // 0x56 'V'
  { 0x11,0x11,0x11,0x11,0x15,0x15,0x15,0x0A }, // 0x57 'W'
  { 0x11,0x11,0x0A,0x04,0x04,0x0A,0x11,0x11 }, // 0x58 'X'
  { 0x11,0x11,0x0A,0x04,0x04,0x04,0x04,0x04 }, // 0x59 'Y'
  { 0x1F,0x01,0x02,0x04,0x04,0x08,0x10,0x1F }, // 0x5A 'Z'
  { 0x0E,0x08,0x08,0x08,0x08,0x08,0x08,0x0E }, // 0x5B '['
  { 0x10,0x10,0x08,0x08,0x04,0x02,0x02,0x01 }, // 0x5C '\\'
  { 0x0E,0x02,0x02,0x02,0x02,0x02,0x02,0x0E }, // 0x5D ']'
  ...
  { 0x00,0x00,0x0E,0x01,0x0F,0x11,0x11,0x0F }, // 0x61 'a'
  { 0x10,0x10,0x10,0x16,0x19,0x11,0x11,0x1E }, // 0x62 'b'
  { 0x00,0x00,0x0E,0x11,0x10,0x10,0x11,0x0E }, // 0x63 'c'
  { 0x01,0x01,0x01,0x0D,0x13,0x11,0x11,0x0F }, // 0x64 'd'
  { 0x00,0x00,0x0E,0x11,0x1F,0x10,0x11,0x0E }, // 0x65 'e'
  { 0x06,0x09,0x08,0x1E,0x08,0x08,0x08,0x08 }, // 0x66 'f'
  { 0x00,0x0E,0x11,0x11,0x11,0x0F,0x01,0x0E }, // 0x67 'g'
  { 0x10,0x10,0x10,0x16,0x19,0x11,0x11,0x11 }, // 0x68 'h'
  { 0x04,0x00,0x0C,0x04,0x04,0x04,0x04,0x0E }, // 0x69 'i'
  { 0x02,0x00,0x06,0x02,0x02,0x02,0x12,0x0C }, // 0x6A 'j'
  { 0x10,0x10,0x12,0x14,0x18,0x14,0x12,0x11 }, // 0x6B 'k'
  { 0x0C,0x04,0x04,0x04,0x04,0x04,0x04,0x0E }, // 0x6C 'l'
  { 0x00,0x00,0x1A,0x15,0x15,0x11,0x11,0x11 }, // 0x6D 'm'
  { 0x00,0x00,0x16,0x19,0x11,0x11,0x11,0x11 }, // 0x6E 'n'
  { 0x00,0x00,0x0E,0x11,0x11,0x11,0x11,0x0E }, // 0x6F 'o'
  { 0x00,0x00,0x1E,0x11,0x11,0x11,0x1E,0x10 }, // 0x70 'p'
  { 0x00,0x00,0x0F,0x11,0x11,0x11,0x0F,0x01 }, // 0x71 'q'
  { 0x00,0x00,0x16,0x19,0x10,0x10,0x10,0x10 }, // 0x72 'r'
  { 0x00,0x00,0x0F,0x10,0x10,0x0E,0x01,0x1E }, // 0x73 's'
  { 0x08,0x08,0x1E,0x08,0x08,0x08,0x09,0x06 }, // 0x74 't'
  { 0x00,0x00,0x11,0x11,0x11,0x11,0x13,0x0D }, // 0x75 'u'
  { 0x00,0x00,0x11,0x11,0x11,0x11,0x0A,0x04 }, // 0x76 'v'
  { 0x00,0x00,0x11,0x11,0x15,0x15,0x15,0x0A }, // 0x77 'w'
  { 0x00,0x00,0x11,0x0A,0x04,0x04,0x0A,0x11 }, // 0x78 'x'
  { 0x00,0x00,0x11,0x11,0x11,0x0F,0x01,0x0E }, // 0x79 'y'
  { 0x00,0x00,0x1F,0x02,0x04,0x04,0x08,0x1F }, // 0x7A 'z'
  { 0x06,0x08,0x04,0x18,0x18,0x04,0x08,0x06 }, // 0x7B '{'
  { 0x04,0x04,0x04,0x04,0x04,0x04,0x04,0x04 }, // 0x7C '|'
  { 0x0C,0x02,0x04,0x03,0x03,0x04,0x02,0x0C }, // 0x7D '}'
  { 0x08,0x15,0x02,0x00,0x00,0x00,0x00,0x00 }, // 0x7E '~'
};

// 한 자리 숫자(0~9) 그리기 — 시계 표시용 (FONT_5X8 공유)
void drawDigit(int x, int y, int d, uint16_t color) {
  if (d < 0 || d > 9) return;
  const uint8_t* glyph = FONT_5X8[(uint8_t)('0' + d) - FONT_5X8_FIRST];
  for (int r = 0; r < 8; r++) {
    uint8_t row = pgm_read_byte(&glyph[r]);
    for (int c = 0; c < 5; c++) {
...
// 클리핑 영역(yMin..yMax) 내부 행만 그린다.
static void drawDigitClipped(int x, int y, int d, uint16_t color, int yMin, int yMax) {
  if (d < 0 || d > 9) return;
  const uint8_t* glyph = FONT_5X8[(uint8_t)('0' + d) - FONT_5X8_FIRST];
  for (int r = 0; r < 8; r++) {
    int yy = y + r;
    ...
    uint8_t row = pgm_read_byte(&glyph[r]);
    for (int c = 0; c < 5; c++) {
      if (row & (1 << (4 - c))) matrix.drawPixel(x + c, yy, color);
    }
  }
}
...
// ASCII 한 글자 그리기 (스크롤 중 x 가 매트릭스 밖이어도 drawPixel 이 알아서 클리핑)
static void drawCharPx(int x, int y, char c, uint16_t color) {
  if (c < FONT_5X8_FIRST || c > FONT_5X8_LAST) return;
  const uint8_t* glyph = FONT_5X8[(uint8_t)c - FONT_5X8_FIRST];
  for (int r = 0; r < FONT_5X8_H; r++) {
    uint8_t row = pgm_read_byte(&glyph[r]);
    for (int col = 0; col < FONT_5X8_W; col++) {
      if (row & (1 << (FONT_5X8_W - 1 - col))) {
        matrix.drawPixel(x + col, y + r, color);
      }
    }
  }
}

// 문자열 그리기 — 다음 글자가 시작될 x 좌표(끝점) 반환
static int drawStringPx(int x, int y, const char* s, uint16_t color) {
  while (*s) {
...
void drawScrollInfo(const struct tm* lt) {
  ...
  // 날짜(녹색) + 요일(평일 녹색 / 토·일·공휴일 적색) — 커스텀 5×8 폰트
  x = drawStringPx(x, 0, datePart, C_GREEN);
  x = drawStringPx(x, 0, dayPart,  colWeekday);

  if (g_w.valid) {
    // 온도 숫자 + 도(°) 기호 2×2 픽셀 (y=0,1 최상단)
    x = drawStringPx(x, 0, prefixNum, colPrefix);
  ...
  x = drawStringPx(x, 0, prefixRest, colPrefix);
  x = drawStringPx(x, 0, suffix,     colSuffix);
...
void drawWiFiQuestion() {
  ...
  const int msgPx = (int)(sizeof(msg) - 1) * FONT_X_ADVANCE;
  if (--sx < -msgPx) sx = MATRIX_W;              // 끝까지 가면 처음으로

  drawStringPx(sx, 0, msg, C_RED);
}
